...
const getOnChangeClientState = propsList => {
    return (
        getInnermostProperty(propsList, HEAD_PROPS.ON_CHANGE_CLIENT_STATE) ||
        (() => {})
    );
};
...
const reducePropsToState = propsList => ({
    ...
    defer: getInnermostProperty(propsList, HEAD_PROPS.DEFER),
    encode: getInnermostProperty(
        propsList,
        HEAD_PROPS.ENCODE_SPECIAL_CHARACTERS
    ),
...
const updateAttributes = (tagName, attributes) => {
    ...
    const headAttributeString = elementTag.getAttribute(HEAD_ATTRIBUTE);
    const headAttributes = headAttributeString
        ? headAttributeString.split(",")
        : [];
    const attributesToRemove = [].concat(headAttributes);
    ...
    if (headAttributes.length === attributesToRemove.length) {
        elementTag.removeAttribute(HEAD_ATTRIBUTE);
    } else if (
        elementTag.getAttribute(HEAD_ATTRIBUTE) !== attributeKeys.join(",")
    ) {
        elementTag.setAttribute(HEAD_ATTRIBUTE, attributeKeys.join(","));
    }
};
...
const updateTags = (type, tags) => {
    ...
    if (tags && tags.length) {
        tags.forEach(tag => {
            ...
            newElement.setAttribute(HEAD_ATTRIBUTE, "true");
...
const generateTitleAsString = (type, title, attributes, encode) => {
    ...
    return attributeString
        ? `<${type} ${HEAD_ATTRIBUTE}="true" ${attributeString}>${encodeSpecialCharacters(
              flattenedTitle,
              encode
          )}</${type}>`
        : `<${type} ${HEAD_ATTRIBUTE}="true">${encodeSpecialCharacters(
              flattenedTitle,
              encode
          )}</${type}>`;
};
...
const generateTagsAsString = (type, tags, encode) =>
    tags.reduce((str, tag) => {
        ...
        return `${str}<${type} ${HEAD_ATTRIBUTE}="true" ${attributeHtml}${isSelfClosing
            ? `/>`
            : `>${tagContent}</${type}>`}`;
    }, "");

const convertElementAttributestoDulcetProps = (attributes, initProps = {}) => {
    return Object.keys(attributes).reduce((obj, key) => {
        obj[DULCET_TAG_MAP[key] || key] = attributes[key];
        return obj;
    }, initProps);
};

const convertDulcetPropstoHtmlAttributes = (props, initAttributes = {}) => {
...
const generateTitleAsDulcetComponent = (type, title, attributes) => {
    // assigning into an array to define toString function on it
    const initProps = {
        key: title,
        [HEAD_ATTRIBUTE]: true
    };
    const props = convertElementAttributestoDulcetProps(attributes, initProps);

    return [Dulcet.createElement(TAG_NAMES.TITLE, props, title)];
};

const generateTagsAsDulcetComponent = (type, tags) =>
    tags.map((tag, i) => {
        const mappedTag = {
            key: i,
            [HEAD_ATTRIBUTE]: true
        ...
        Object.keys(tag).forEach(attribute => {
            const mappedAttribute = DULCET_TAG_MAP[attribute] || attribute;
        ...
        return Dulcet.createElement(type, mappedTag);
    });
...
const getMethodsForTag = (type, tags, encode) => {
    switch (type) {
        case TAG_NAMES.TITLE:
            return {
                toComponent: () =>
                    generateTitleAsDulcetComponent(
                        type,
                        tags.title,
        ...
        case ATTRIBUTE_NAMES.HTML:
            return {
                toComponent: () => convertElementAttributestoDulcetProps(tags),
                toString: () => generateElementAttributesAsString(tags)
            };
        default:
            return {
                toComponent: () => generateTagsAsDulcetComponent(type, tags),
...
export {convertDulcetPropstoHtmlAttributes};
