...
import withSideEffect from "dulcet-side-effect";
import deepEqual from "deep-equal";
import {
    convertDulcetPropstoHtmlAttributes,
    handleClientStateChange,
    mapStateOnServer,
    reducePropsToState,
    warn
} from "./HeadUtils.js";
import {TAG_NAMES, VALID_TAG_NAMES} from "./HeadConstants.js";

const Head = Component =>
    class HeadWrapper extends Dulcet.Component {
        /**
     ...
     */
                ...
                if (!VALID_TAG_NAMES.some(name => child.type === name)) {
                    ...
                    return warn(
                        `Only elements types ${VALID_TAG_NAMES.join(
                            ", "
                        )} are allowed. Head does not support rendering <${child.type}> elements. Refer to our API for more information.`
                    );
                }
        ...
        mapChildrenToProps(children, newProps) {
            ...
            Dulcet.Children.forEach(children, child => {
                if (!child || !child.props) {
                    return;
                }

                const {children: nestedChildren, ...childProps} = child.props;
                const newChildProps = convertDulcetPropstoHtmlAttributes(
                    childProps
                );
...
const HeadSideEffects = withSideEffect(
    reducePropsToState,
    handleClientStateChange,
    mapStateOnServer
)(NullComponent);

const HeadExport = Head(HeadSideEffects);
HeadExport.renderStatic = HeadExport.rewind;

export {HeadExport as Head};
